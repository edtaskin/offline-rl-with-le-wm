...
def binary_curve_metrics(
    ...
    positive_count = y.sum(axis=0).astype(np.float32)
    negative_count = (~y).sum(axis=0).astype(np.float32)
    ...
    false_negative_rate = fn / np.maximum(fn + tp, 1.0)
    p_on_positive = np.divide(
        (p * y).sum(axis=0),
        positive_count,
        out=np.full_like(positive_count, np.nan, dtype=np.float32),
        where=positive_count > 0,
    )
    p_on_negative = np.divide(
        (p * ~y).sum(axis=0),
        negative_count,
        out=np.full_like(negative_count, np.nan, dtype=np.float32),
        where=negative_count > 0,
    )
    return {
        "objective_met_positive_count": positive_count,
        "objective_met_negative_count": negative_count,
        "objective_met_positive_rate": y.mean(axis=0),
        "objective_met_mean_probability": p.mean(axis=0),
        "objective_met_mean_probability_on_positives": p_on_positive,
        "objective_met_mean_probability_on_negatives": p_on_negative,
        "objective_met_predicted_rate": pred.mean(axis=0),
        "objective_met_true_positives": tp,
        "objective_met_true_negatives": tn,
        "objective_met_false_positives": fp,
        "objective_met_false_negatives": fn,
        "objective_met_false_positive_rate": false_positive_rate,
        "objective_met_false_negative_rate": false_negative_rate,
...
def plot_objective_reliability_curves(
    path: Path,
    env_steps: np.ndarray,
    imagined: dict[str, np.ndarray],
    encoded_gt: dict[str, np.ndarray],
    ...
    fig, axes = plt.subplots(2, 2, figsize=(12, 8), constrained_layout=True)
    specs = [
        ("objective_met_positive_count", "GT Objective-Met Support", "Count"),
        ("objective_met_recall", "Recall on GT Objective-Met States", "Rate"),
        ("objective_met_false_negative_rate", "False Negative Rate on GT Objective-Met States", "Rate"),
        ("objective_met_precision", "Precision of Objective-Met Predictions", "Rate"),
    ]
    for ax, (key, title, ylabel) in zip(axes.flat, specs):
        ...
        ax.set_ylabel(ylabel)
        ax.grid(True, alpha=0.25)
        if key != "objective_met_positive_count":
            ax.set_ylim(0.0, 1.05)
    axes[0, 1].legend()
    path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(path, dpi=180)
...
def main() -> None:
    ...
    plot_objective_reliability_curves(
        output_dir / "rollout_probe_objective_reliability.png",
        env_steps,
        imagined_curves,
        encoded_gt_curves,
    )
    ...
    summary = {
        ...
        "final_step": {
            ...
            or key.endswith("_false_negative_rate")
            or key.endswith("_false_positives")
            or key.endswith("_false_negatives")
            or key.endswith("_positive_count")
            or key.endswith("_negative_count")
            or key.endswith("_mean_probability")
            or key.endswith("_mean_probability_on_positives")
            or key.endswith("_mean_probability_on_negatives")
