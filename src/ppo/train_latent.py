...
``checkpoints/trained_policies/pusht_latent_bc.pth``). Runnable either way::
    ...
    python -m src.ppo.train_latent --smoke
    python src/ppo/train_latent.py --fixed_target --frame_stack 3 --action_chunk_size 5
...
# Allow ``python src/ppo/train_latent.py`` (not just ``-m``) by putting the repo
...
def main() -> None:
    ...
    from src.ppo.latent_ppo import LatentPPOTrainer
