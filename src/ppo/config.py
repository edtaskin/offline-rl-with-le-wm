...
class LatentConfig:
    ...
    # Frozen feature consumed by the policy. The PPO entry points restore this
    # from BC stats so the checkpoint cannot silently receive the other 192-d
    # representation.
    ...
    def __post_init__(self) -> None:
        ...
        if self.latent_representation not in LEWM_LATENT_REPRESENTATIONS:
            raise ValueError(
                f"unsupported LeWM latent representation: {self.latent_representation!r}"
            )
