...
Mirrors the from-scratch, CleanRL-style structure of :mod:`src.ppo.ppo`, but the
observation is an image and the "action" is an open-loop chunk of
``action_chunk_size`` env steps:
...
* The frozen encoder turns each frame into a latent once, during rollout; the
  dilated stack of latents (see :class:`src.ppo.latent_env.LatentHistory`) is the
...
* Time-limit truncation (not success) bootstraps ``gamma**(j+1) * V(terminal)``
  before the env is reset, exactly like :mod:`src.ppo.ppo`.
"""
...
from src.ppo.latent_config import LatentConfig
...
class LatentPPOTrainer:
    def __init__(self, cfg: LatentConfig, encoder=None):
        ...
        self.writer = None  # optional wandb run, set in train_latent.py
    ...
    def _evaluate_heldout(self) -> dict:
        ...
        Mirrors src/ppo/evaluate_latent.py: open-loop chunk execution with the
        ...
        if self._eval_env is None:
            ...
            # matches the coef=0 env that evaluate_latent.py uses. The block
