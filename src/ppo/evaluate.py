...
Evaluator for the frozen-encoder latent agent (:mod:`src.ppo.agent`). One policy
decision is an open-loop chunk of ``action_chunk_size`` env steps; the policy
input is the dilated stack of per-frame LeWM latents
(:class:`src.ppo.env.LatentHistory`), exactly as during training. Env settings
(``fixed_target``, ``max_episode_steps``, ...) and the agent contract are read
from the checkpoint so evaluation matches training::
...
def run_evaluation(
    *,
    ...
    The evaluation harness behind :func:`evaluate`: identical env, dilated
    ``LatentHistory``, replanning strategy, success metric, and summary. All
    config is already resolved by the caller (no checkpoint defaults are
    consulted here).
    """
    k = int(contract["action_chunk_size"])
